...
- `手动平仓`：对当前持仓执行市价卖出；实盘模式会先弹出二次确认。

设置页已按功能拆成多个子项：

- `基础`：计价币种、单笔金额、最大持仓数、轮询间隔、状态文件。
- `入场筛选`：涨幅、波动、成交额、热门帖子/币种数量、白名单/黑名单、大盘过滤。
- `退出风控`：初始止损、止盈、保本、移动止盈、固定金额止损、权益触发。
- `交易安全`：最小下单/卖出金额、手续费、滑点、成交后账户同步。
- `运行限制`：冷却时间、每日开仓上限、每日最大亏损、Testnet/Live/浏览器抓取。
...
仓库已加入 `.editorconfig` 和 `.gitattributes`，用于统一编辑器字符集和 Git 换行处理。
